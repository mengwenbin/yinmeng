...
public abstract class HXSDKHelper {
	...
	/**
	 * init标记:测试如果sdk已经请来之前我们不需要初始化一次
	 */
	...
	public synchronized boolean onInit(Context context) {
		...
		// 如果子类创建一个defalut HX SDK模型没有提供模型
		if (hxModel == null) {
			hxModel = new YMHXSDKModel(appContext);
		}
	...
	/**
	 * 子类覆盖这个类必须提供自己的模型或直接使用{ @link DemoHXSDKModel }
	 * @return
	 */
	abstract protected HXSDKModel createModel();

	/**
	 * 请确保你得EMChatOptions遵循方法和设置相关选项EMChatOptions 选项 = EMChatManager.getInstance().getChatOptions();
	 */
	...
	/**
	 * 开发人员可以重写这个函数来处理连接冲突错误
	 */
	protected void onConnectionConflict() {
	}

	/**
	 * 开发人员可以重写这个函数来处理用户删除错误
	 */
	protected void onCurrentAccountRemoved() {
	}

	/**
	 * 处理连接connectedhandle连接连接
	 */
