...
public class SortWeekFragment extends BaseNewFragment implements
	...
	private final  String mPageName = "SortWeekFragment";
	...
	public int setLayoutId() {
		...
		return R.layout.fragment_sort_week;
	}
	...
	public void setupView(View view) {
		...
		tv_week = getTextView(view, R.id.tv_week);
		btn_select = getImageButton(view, R.id.btn_select);
		btn_select.setOnClickListener(this);
		layout_select = getRelativeLayout(view, R.id.layout_select);
		gv_week = getGridView(view, R.id.gridView_week);
		gv_week.setOnItemClickListener(this);
		
		listview = (XListView) view.findViewById(R.id.listview_sort_week);
	...
	public void setModel() {
		...
		nowWeek = TimeUtil.getWeekOfYear(System.currentTimeMillis());
		currentWeek = nowWeek;
		currentYear = nowYear;
		if(nowYear == 2015){
			adapter_time = new SortListSelectTimeAdapter(context, nowWeek, Constant.INIT_WEEK, "周");
		}else{
			adapter_time = new SortListSelectTimeAdapter(context, nowWeek, 1, "周");
		}
		gv_week.setAdapter(adapter_time);
		tv_week.setText(currentYear+"年 "+currentWeek+"周");
	...
	OnItemClickListener listener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			getParentFragment().getParentFragment().startActivityForResult(in, 12);
		}
	};
	...
	public void onClick(View v) {
		...
		case R.id.btn_select:
			if(isshow){
				...
				adapter_time.setSelect(currentWeek);
	...
	public void task(){
		if (haveNetwork(context)) {
			SortListTask task = new SortListTask(context, String.valueOf(music_category), String.valueOf(currentYear), String.valueOf(currentWeek), callback);
			task.excute();
		} else {
			if (adapter != null) {
