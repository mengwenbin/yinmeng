...
public class MusicUserAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if("1".equals(list.get(position).getIs_attention())){
			holder.btn_attention.setImageResource(R.drawable.btn_cancle_attention);
		}else if("0".equals(list.get(position).getIs_attention())){
		...
		public void onClick(View v) {
			if(AccountInfoUtils.getInstance(context).isLogin()){
				if(haveNetwork()){
					...
					if("0".equals(userInfo.getIs_attention())){
					...
					}else if("1".equals(userInfo.getIs_attention())){
