...
public class UploadHeadTask extends BaseUploadTask {
	/**
	 * 上传用户头像
	 * @param context
	 * @param filePath
	 * @param callback
	 */
	public UploadHeadTask(Context context, String filePath,
		...
		params.addBodyParameter("user_img", new File(filePath));
		url = APIUrl.BASE_URL + APIUrl.UPLOAD_HEAD;
	}
	/*
	 * 上传封面图
	 */
	public UploadHeadTask(Context context, String filePath,String picname,
			RequestCallBack<String> callback) {
		super(context, callback);
		params.addBodyParameter("access_token", APIUrl.ACCESS_TOKEN);
		params.addBodyParameter("token",AccountInfoUtils.getInstance(context).getAccountToken());
		params.addBodyParameter("user_id",AccountInfoUtils.getInstance(context).getAccountId());
		params.addBodyParameter("Attachment[filename]", picname);
		params.addBodyParameter("Attachment[attachment]", new File(filePath));
		url = APIUrl.BASE_URL + APIUrl.UPLOAD_COVER;
	}
